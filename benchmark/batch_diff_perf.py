"""Quick perf comparison: DifferentialController vs BatchDifferentialController at scale.

Spawns N differential-drive agents on a grid, gives each a single short path,
and measures wall time + per-step phase costs over a fixed step budget.

Run::

    python3 benchmark/batch_diff_perf.py --n 500 --steps 600

By default collision is on at every step (collision_check_frequency=60), since
that matches the typical PyBulletFleet use case.
"""
...
def _make_sim(collision_freq: int) -> MultiRobotSimulationCore:
...
def _build_waypoints(start: Pose, n_wp: int = 3, leg: float = 0.4) -> list[Pose]:
    """Each waypoint forces a small rotation + translation."""
    out: list[Pose] = []
    for i in range(1, n_wp + 1):
        # Zigzag in xy to make the differential drive rotate per waypoint.
        dx = leg * i
...
def bench_per_agent(n: int, steps: int, collision_freq: int) -> dict:
    ...
    return _run(sim, steps, label=f"per-agent DifferentialController, n={n}, col_freq={collision_freq}")
...
def bench_batch(n: int, steps: int, collision_freq: int) -> dict:
    ...
    for a in agents:
        ...
        wps = _build_waypoints(a.get_pose())
        bc.set_path(a, wps)
    return _run(sim, steps, label=f"BatchDifferentialController, n={n}")
...
def main() -> None:
    ...
    print(
        f"\n=== Batch vs per-agent DifferentialController perf  "
