...
def test_multi_cell_with_agents():
    ...
    for i in range(3):
        ...
        agent = sim_core.spawn_agent(spawn_params)
    ...
    from pybullet_fleet.sim_object import Structure
    
    # Create large structure (10m x 10m plane)
    large_structure = sim_core.add_structure(
        mesh_path="mesh/plane.obj",
        position=[10, 10, 0],
        orientation=[0, 0, 0, 1],
        scale=[5.0, 5.0, 1.0],  # Scale to make it large
        mass=0.0,
