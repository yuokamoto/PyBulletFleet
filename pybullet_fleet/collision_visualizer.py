...
class CollisionVisualizer:
    ...
    def draw_body_collision_shapes(self, body_id: int, color: List[float] = [0, 1, 0]) -> None:
        ...
        for link_index in links_to_check:
            ...
            for shape_info in collision_data:
                ...
                world_pos, world_orn = p.multiplyTransforms(link_pos, link_orn, local_frame_pos, local_frame_orn)
    ...
    def _draw_box(self, pos, orn, half_extents, color):
        ...
        edges = [
            ...
            (2, 3),
            (4, 5),
            (6, 7),  # edges parallel to x
            (0, 2),
            (1, 3),
            (4, 6),
            (5, 7),  # edges parallel to y
            (0, 4),
            (1, 5),
            (2, 6),
            (3, 7),  # edges parallel to z
        ]
    ...
    def _draw_aabb(self, aabb_min, aabb_max, color, simplified=True):
        ...
        if simplified:
            ...
            edges = [
                ...
                (1, 2),
                (2, 3),
                (3, 0),  # bottom face (xy plane)
                (4, 5),
                (5, 6),
                (6, 7),
                (7, 4),  # top face (xy plane)
                (0, 4),
                (1, 5),
                (2, 6),
                (3, 7),  # vertical edges (z direction)
            ]
        else:
