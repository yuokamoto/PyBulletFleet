...
Robot management class for controlling multiple.
Designed to be eventually migrated to a ROS2 node.
"""
import numpy as np
from dataclasses import dataclass, field, replace
from typing import List, Dict, Optional, Tuple, Callable, Any
from .robot import Robot, RobotPose, RobotSpawnParams
...
class RobotManager:
    """
    Manager class for multiple robots.
    
    This class provides centralized control of multiple robots and is designed
    to eventually become a ROS2 node that:
    - Publishes robot poses (geometry_msgs/Pose)
    - Subscribes to goal poses (geometry_msgs/PoseStamped)
    
    Features:
    - Grid-based robot spawning
    - Pose goal assignment to individual robots
    - Query all robot poses
    - Update all robots in a single call
    - Callback system for custom goal update logic
    
    Note: Robot-specific state is stored in each Robot.user_data dict
    """
    ...
    def __init__(self, sim_core=None):
        ...
        self.robot_ids: Dict[int, Robot] = {}  # body_id -> robot mapping
    ...
    def spawn_robots_grid(self, num_robots: int,
                         ...
                         spawn_params: RobotSpawnParams) -> List[Robot]:
        """
        ...
        Args:
            ...
            spawn_params: RobotSpawnParams instance with robot parameters
        ...
        # Validate that mesh_path is provided in spawn_params
        ...
        print(f"[RobotManager] Spawning {num_robots} robots in grid pattern...")
            ...
            if current_x > x_max:
                ...
                if current_y > y_max:
                    ...
                    if current_z > z_max:
                        ...
                        if i < num_robots - 1:
                            print(f"[RobotManager] Warning: Grid exhausted after {i + 1} robots "
        ...
        print(f"[RobotManager] Successfully spawned {len(spawned_robots)} robots!")
        return spawned_robots
    
    def set_goal_pose(self, robot_index: int, goal: RobotPose):
        """
        Set goal pose for a specific robot.
        
        Args:
            robot_index: Index of robot in self.robots list
            goal: Target RobotPose
        """
        if 0 <= robot_index < len(self.robots):
            self.robots[robot_index].set_goal_pose(goal)
        else:
            print(f"[RobotManager] Warning: Invalid robot index {robot_index}")
    
    def set_goal_pose_by_id(self, body_id: int, goal: RobotPose):
        """
        ...
        if body_id in self.robot_ids:
            ...
            print(f"[RobotManager] Warning: Unknown robot body_id {body_id}")
    
    def get_pose(self, robot_index: int) -> Optional[RobotPose]:
        """
        ...
        Returns:
            Current RobotPose or None if invalid index
        """
        if 0 <= robot_index < len(self.robots):
    ...
    def get_all_poses(self) -> List[RobotPose]:
        """
        Get current poses of all robots.
        
        Returns:
            List of RobotPose objects for all robots
        """
        return [robot.get_pose() for robot in self.robots]
    
    def get_poses_dict(self) -> Dict[int, RobotPose]:
        """
        Get current poses of all robots as a dictionary.
        
        Returns:
            Dictionary mapping body_id to RobotPose
        """
    ...
    def register_goal_update_callback(self, callback: Callable[[List[Robot], 'RobotManager', float], None]):
        """
        ...
        The callback will be called during update_all_with_goals() and should have signature:
            callback(robots: List[Robot], manager: RobotManager, dt: float) -> None
    ...
    def __repr__(self):
        return (f"RobotManager(total={len(self.robots)}, "
