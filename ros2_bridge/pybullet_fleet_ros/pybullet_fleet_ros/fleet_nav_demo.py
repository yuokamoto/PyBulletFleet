"""Send non-overlapping batched navigation goals through the Fleet ROS API."""
...
def _make_goals(
    positions: dict[str, tuple[float, float, float]], *, count: int, dx: float, dy: float, yaw: float
) -> list[RobotGoal2D]:
    return [
        RobotGoal2D(name=name, position=[x + dx, y + dy], yaw=yaw, z=z)
        for name, (x, y, z) in sorted(positions.items())[:count]
    ]


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--robots", type=int, default=10, help="Number of robots to command")
